...
export const CartProvider = ({ children }) => {
  ...
  const getProducts = async () => {
    await axios
      ...
      .get("https://tfg-ferrangaldon-backend.onrender.com/products")
  ...
  const getProductsCart = async () => {
    return await axios
      ...
      .get("https://tfg-ferrangaldon-backend.onrender.com/products-cart")
  ...
  const addItemToCart = async (product) => {
    ...
    await axios.post("https://tfg-ferrangaldon-backend.onrender.com/products-cart", { name, img, price, size });
  ...
  const editItemToCart = async (id, query, amount) => {
    if (query === "del" && amount === 1) {
      await axios
        ...
        .delete(`https://tfg-ferrangaldon-backend.onrender.com/products-cart/${id}`)
        .then(({ data }) => console.log(data));
    } else {
      await axios
        //.put(`http://localhost:4000/products-cart/${id}?query=${query}`, {
         .put(`https://tfg-ferrangaldon-backend.onrender.com/products-cart/${id}?query=${query}`, {
          amount,
        })
        .then(({ data }) => console.log(data));
    }
